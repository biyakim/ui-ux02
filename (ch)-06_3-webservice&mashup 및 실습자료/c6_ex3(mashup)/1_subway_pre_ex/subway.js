...
function handleRefresh() {
    console.log("here");
    var url = "http://openapi.seoul.go.kr:8088/키연결/데이터타입/서비스명/1/2/"+test+"/1/1/"
    $.getJSON(url + "?callback=", updateTraffic);
	var url = "http://openapi.seoul.go.kr:8088/키연결/데이터타입/서비스명/1/2/"+test+"/1/2/"
	$.getJSON(url + "?callback=", updateTraffic);
}

function updateTraffic(subways) {
	var traffic = subways.키연결.row;
